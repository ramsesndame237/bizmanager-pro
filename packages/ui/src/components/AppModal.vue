...
withDefaults(
  defineProps<{ open: boolean; title?: string; size?: ModalSize; closable?: boolean }>(),
  ...
  <Teleport to="body">
    <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
      <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center p-4" aria-modal="true" role="dialog">
        <div class="absolute inset-0 bg-black/50" @click="closable && emit('close')" />
        <div :class="['relative w-full bg-white rounded-xl shadow-xl', sizeClasses[size]]">
          <div v-if="title || closable" class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            ...
            <div v-else />
            <button v-if="closable" class="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 transition-colors" aria-label="Close" @click="emit('close')">
